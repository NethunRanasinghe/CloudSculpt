...
public class SettingsVmSaveCommand(SettingsV2ViewModel settingsV2ViewModel) : CommandBase
{
    public override async void Execute(object? parameter)
    {
        ...
        if (!isExist)
        {
            var configName = settingsV2ViewModel.VmConfigName;
            var configIp = settingsV2ViewModel.VmIpAddress;
            ...
            var box = MessageBoxManager
                .GetMessageBoxStandard("Warning", $"Are you sure you want to save the configuration with name:  '{configName}' and ip : '{configIp}' ?",
                    ButtonEnum.YesNo,Icon.Warning);

            var result = await box.ShowAsync();
            if (result == ButtonResult.No) return;
            
            var rows =await DatabaseManage.InsertVmData(configName, configIp);
            if(rows <= 0) return;
